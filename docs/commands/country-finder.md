...
Collects requirements for both tracks. Remote track: maximum timezone difference (or "no limit" to skip). Sponsorship track: relocation timeline. Relocation is assumed; the question is not asked. Country preferences: any countries or regions to include or exclude from both tracks. Vague answers are rejected; specific values or explicit "no limit" / "not specified" are required. It also creates or completes `situational-profile.md` with current location, citizenship-specific immigration facts, languages, salary-floor context, work authorization, age, and the employment basis Salary Calculator should use. The file is reused only when it clearly belongs to the same candidate.
