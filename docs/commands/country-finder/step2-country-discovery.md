...
```mermaid
flowchart TD
  Start([Step 2 begins]) --> IncludeCheck{Include list\ngiven in Step 1?}
  IncludeCheck -->|yes| BaseInclude[Base = Include list]
  IncludeCheck -->|no| AskDefault([Show shipped default list,\nask: proceed with default,\nor provide your own? Wait.])
  AskDefault -->|proceeds with default| BaseDefault[Base = default-preferred-countries.md]
  AskDefault -->|provides own list| BaseOwn[Base = provided list]

  BaseInclude --> ExcludeStep
  BaseDefault --> ExcludeStep
  BaseOwn --> ExcludeStep
...
1. **Base list**: the Step 1 Include list if one was given; otherwise Claude shows you the shipped `data/default-preferred-countries.md` grouped by continent and asks whether to proceed with it or provide your own countries instead, waiting for your answer either way.
2. If an Exclude list was given, those countries are removed from the resulting base.

Other than the default-list question above, this all happens silently; Claude never announces which source it's using as the base or asks whether to use it. The resulting final country list is shown before research begins.

## Part B: Research sub-agents

The final list is batched by region, since `data/default-preferred-countries.md` is organized under region headings; one sub-agent per region present in the final list, when the base came from it. If, for some reason, the list has no usable region grouping (e.g. the base came from your own Include list instead), it falls back to groups of roughly 15 countries. One isolated sub-agent is spawned per batch.
