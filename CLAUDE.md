...
- Country Finder step2's candidate-universe base is the Step 1 Include list if one was given (applied silently), otherwise the shipped `data/default-preferred-countries.md` — the one exception to "silent," since Claude shows it and asks whether to proceed with it or provide different countries, and waits, rather than ever applying it silently. There is no personal/workspace-level override file anymore.
